...
async def get_me(user_id: UUID = Depends(get_route_user_id)):
    user = await _user_profile_service.get_profile(user_id)
    if user is None:
...
async def update_me(
    ...
    user = await _user_profile_service.update_profile(user_id, data.model_dump(exclude_unset=True))
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return user
...
async def list_preferences(user_id: UUID = Depends(get_route_user_id)):
    prefs = await _preference_service.list_preferences(user_id)
...
async def get_preference(key: str, user_id: UUID = Depends(get_route_user_id)):
    pref = await _preference_service.get_preference(user_id, key)
    if pref is None:
...
async def set_preference(
    ...
    pref = await _preference_service.set_preference(user_id, key, data.value)
    return PreferenceResponse(key=pref.key, value=pref.value)
...
async def delete_preference(key: str, user_id: UUID = Depends(get_route_user_id)):
    deleted = await _preference_service.delete_preference(user_id, key)
    if not deleted:
        raise HTTPException(status_code=404, detail="Preference not found")
