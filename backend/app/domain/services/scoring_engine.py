...
from .scoring_strategies import GlobalScoringStrategy, IScoringStrategy
...
class ScoringEngine(IScoringEngine):
    ...
    def score_dimension(self, dimension: DimensionType, data: dict[str, Any], market: str) -> float:
        ...
        for key, value in data.items():
            ...
            if "rsi" in key:
                scores.append(strategy.score_rsi(value))
            elif "pe_ratio" in key:
                scores.append(strategy.score_pe(value))
            else:
                scores.append(min(100.0, max(0.0, float(value))))
