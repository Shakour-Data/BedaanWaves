...
class FakeAsyncSession:
    ...
    def _apply_single(self, rows, crit):
        ...
        if op is operators.is_:
            ...
            rname = type(right).__name__
            if rname == "False_" or target is False:
                return [r for r in rows if getattr(r, crit.left.key, None) is False]
            if rname == "True_" or target is True:
                return [r for r in rows if getattr(r, crit.left.key, None) is True]
            return [r for r in rows if getattr(r, crit.left.key, None) is None]
        return rows
