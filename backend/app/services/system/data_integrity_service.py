...
class DataIntegrityService(BaseService):
    ...
    async def run_full_integrity_check(self) -> dict[str, Any]:
        ...
        report = {
            "check_id": f"integrity_{int(datetime.now(UTC).timestamp())}",
        ...
        try:
            ...
            self.logger.error(f"Error during integrity check: {e!s}")
            report["error"] = str(e)
        ...
        for symbol, asset_type in test_cases:
            ...
            try:
                ...
                if data and "timestamp" in data:
                    ...
                    age_hours = (datetime.now(UTC) - data_time.replace(tzinfo=None)).total_seconds() / 3600
    ...
    def _generate_summary(self, checks: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "total_checks_performed": len([k for k in checks if k not in ["error"]]),
    ...
    def _count_issues(self, checks: dict[str, Any], level: str) -> int:
        ...
        for check_name, check_result in checks.items():
            if isinstance(check_result, dict):
                if check_result.get("status") == "error" or not check_result.get("is_valid"):
                    count += 1
        return count
