...
class SettingsMigrationService(BaseService):
    ...
    async def backup_user_settings(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            backup = {
                ...
                "settings": settings,
                "backup_id": f"backup_{user_id}_{int(datetime.now(UTC).timestamp())}",
            ...
            self.logger.error(f"Error backing up settings for {user_id}: {e!s}")
            return {
                "success": False,
                "error": str(e),
                "user_id": user_id
    ...
    async def restore_user_settings(self,
        ...
        try:
            ...
            self.logger.error(f"Error restoring settings for {user_id}: {e!s}")
            return {
                "success": False,
                "error": str(e),
                "user_id": user_id
    ...
    async def migrate_user_settings(self,
        ...
        try:
            ...
            self.logger.error(f"Error migrating settings for {user_id}: {e!s}")
            return {
                "success": False,
                "error": str(e),
                "user_id": user_id
            ...
            for field, transformer in mapping["transform"].items():
                if field in migrated:
                    try:
                        ...
                        self.logger.warning(f"Error transforming {field}: {e!s}")
