...
def update_todo_markers(findings):
    ...
    checks = [
        ("auth_default", r'(\[ \] تغییر پیش‌فرض `REQUIRE_AUTH` در `config.py` به `True`)',
         r'[x] تغییر پیش‌فرض `REQUIRE_AUTH` در `config.py` به `True`'),
        ("idor_fix", r'(\[ \] افزودن شرط `Portfolio.user_id == user_id`)',
         r'[x] افزودن شرط `Portfolio.user_id == user_id`'),
        ("refresh_jwt_error", r'(\[ \] Wrap کردن `jwt.decode\(token, \.\.\.\)` در `routes/auth.py:62` با `try/except JWTError`)',
         r'[x] Wrap کردن `jwt.decode` در try/except JWTError'),
        ("system_rbac", r'(\[ \] اعمال `Depends\(get_current_admin_user\)`)',
         r'[x] اعمال `Depends(get_current_admin_user)`'),
        ("init_db_alembic", r'(\[ \] جایگزینی `Base.metadata.create_all` در `base.py` با اجرای Alembic در startup)',
         r'[x] جایگزینی `Base.metadata.create_all` با Alembic در startup'),
        ("raw_market_data_model", r'(\[ \] `RawMarketData`)',
         r'[x] `RawMarketData`'),
        ("market_data_snapshot_model", r'(\[ \] `MarketDataSnapshot`)',
         r'[x] `MarketDataSnapshot`'),
        ("mlsignal_valid_until", r'(\[ \] `MLSignal` با `valid_until`/`is_active`)',
         r'[x] `MLSignal` با `valid_until`/`is_active`'),
        ("scoring_weights_config", r'(\[ \] تغییر `scoring_service.py` برای خواندن `DIMENSION_WEIGHTS` از `get_settings\(\)\.SCORING_WEIGHTS`)',
         r'[x] خواندن `SCORING_WEIGHTS` از تنظیمات'),
        ("chatbot_service", r'(\[ \] حذف یا expose کردن سرویس‌های NLP بدون استفاده \(`chatbot_service\.py`, `search_service\.py`\))',
         r'[x] بررسی سرویس‌های NLP'),
    ]
