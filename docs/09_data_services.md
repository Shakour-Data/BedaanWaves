...
pipeline_steps = [
    ...
    {"source": "crypto", "task": "fetch_prices"},
...
**File**: `app/services/data/crypto_api_client.py`

### Purpose
Integration with major cryptocurrency exchanges for price data, order book information, and trading data. Supports Binance, Kraken, Coinbase, and other major exchanges.

### Key Features
- Multi-exchange price aggregation
- Order book data retrieval
- Ticker and symbol management
...
| Exchange | APIs Supported | Features |
|----------|--------------|----------|
| Binance | REST + WebSocket | Spot, futures, margin |
| Kraken | REST + WebSocket | Spot, futures |
| Coinbase | REST + WebSocket | Spot, advanced |
| KuCoin | REST + WebSocket | Spot, futures |

### Trading Pair Support
```python
# Get trading pairs
pairs = await crypto_client.get_trading_pairs({
    "exchange": "binance",
    "base_assets": ["BTC", "ETH", "USDT"]
})

# Returns: ["BTCUSDT", "ETHUSDT", "BTCUSDT_MARGIN", ...]

# Get pair details
details = await crypto_client.get_pair_details({
    "symbol": "BTCUSDT",
    "exchange": "binance"
})
...
prices = await crypto_client.get_aggregated_prices({
...
history = await crypto_client.get_crypto_history({
    "symbol": "BTCUSDT",
    "exchange": "binance",
...
client = CryptoApiClient(
    ...
    api_keys={
        "binance": {"key": "binance-key", "secret": "binance-secret"},
        "kraken": {"key": "kraken-key", "secret": "kraken-secret"}
    }
)

# Get BTC price from multiple exchanges
prices = await client.get_aggregated_prices({"symbols": ["BTCUSDT"]})

# Get historical BTC data
history = await client.get_crypto_history({
    "symbol": "BTCUSDT",
    "exchange": "binance",
    "start": "2024-01-01",
    "end": "2024-08-17",
...
available = await client.check_exchange_health({"exchange": "binance"})
```

### Key Methods
| Method | Description |
|--------|-------------|
| `get_aggregated_prices(symbols, exchanges)` | Get prices from multiple exchanges |
| `get_crypto_history(symbol, exchange, start, end, interval)` | Historical crypto data |
| `get_trading_pairs(exchange, base_assets)` | Get available pairs |
| `get_pair_details(symbol, exchange)` | Get pair specifications |
| `check_exchange_health(exchange)` | Check exchange availability |
| `get_ticker_info(symbol, exchange)` | Get ticker metadata |
