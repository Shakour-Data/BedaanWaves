...
## Overview
This document describes the REST API endpoints for the BedaanWaves platform. The API is organized into logical groups under `/api/v1/` prefixes.
...
- **GET /api/v1/health/services** - Detailed health status for all registered services
- **GET /api/v1/health/services/{service_name}** - Health status for a specific service
...
**Note**: Additional API documentation is available in the source files:
- docs/api/postgre.md - PostgreSQL-specific API notes
- docs/api/BourseApi.md - Bourse API integration details