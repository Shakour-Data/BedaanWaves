...
**توضیح:** در این سطح،_guard_ها مقدار درخواست، مانده `BudgetAllocation`، نسخه اصلاح و تطابق فاکتور با رسید را کنترل می‌کنند. دریافت ناقص به حالت `PartiallyReceived` می‌رود و کسری کالا می‌تواند سفارش جدید ایجاد کند. همه گذارهای مهم رویداد `AuditLog` تولید می‌کنند.
...
4. وضعیت‌های `Paid` و `Reconciled` غیرقابل بازگشت هستند، مگر از طریق جبران خطای `Closed` نیاز به‌کننده‌های مدیریتی است.
